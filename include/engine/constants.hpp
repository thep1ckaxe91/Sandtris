...
namespace sdlgame
{
    ...
    const Uint32 SKIP_TASK_BAR = SDL_WINDOW_UTILITY;
    ...
    const Uint32 QUIT = SDL_EVENT_QUIT;
    const Uint32 KEYUP = SDL_EVENT_KEY_UP;
    const Uint32 KEYDOWN = SDL_EVENT_KEY_DOWN;
    const Uint32 MOUSEBUTTONDOWN = SDL_EVENT_MOUSE_BUTTON_DOWN;
    const Uint32 MOUSEBUTTONUP = SDL_EVENT_MOUSE_BUTTON_UP;
    const Uint32 MOUSEMOTION = SDL_EVENT_MOUSE_MOTION;
    const Uint32 MOUSEWHEEL = SDL_EVENT_MOUSE_WHEEL;
    const Uint32 USEREVENT = SDL_EVENT_USER;

    const Uint32 WINDOWFIRST = SDL_EVENT_WINDOW_FIRST;
    const Uint32 WINDOWSHOWN = SDL_EVENT_WINDOW_SHOWN;              // Window became shown
    const Uint32 WINDOWHIDDEN = SDL_EVENT_WINDOW_HIDDEN;            // Window became hidden
    const Uint32 WINDOWEXPOSED = SDL_EVENT_WINDOW_EXPOSED;          // Window got updated by some external event
    const Uint32 WINDOWMOVED = SDL_EVENT_WINDOW_MOVED;              // Window got moved
    const Uint32 WINDOWRESIZED = SDL_EVENT_WINDOW_RESIZED;          // Window got resized
    const Uint32 WINDOWSIZECHANGED = SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED; // Window changed its size
    const Uint32 WINDOWMINIMIZED = SDL_EVENT_WINDOW_MINIMIZED;      // Window was minimized
    const Uint32 WINDOWMAXIMIZED = SDL_EVENT_WINDOW_MAXIMIZED;      // Window was maximized
    const Uint32 WINDOWRESTORED = SDL_EVENT_WINDOW_RESTORED;        // Window was restored
    const Uint32 WINDOWENTER = SDL_EVENT_WINDOW_MOUSE_ENTER;              // Mouse entered the window
    const Uint32 WINDOWLEAVE = SDL_EVENT_WINDOW_MOUSE_LEAVE;              // Mouse left the window
    const Uint32 WINDOWFOCUSGAINED = SDL_EVENT_WINDOW_FOCUS_GAINED; // Window gained focus
    const Uint32 WINDOWFOCUSLOST = SDL_EVENT_WINDOW_FOCUS_LOST;     // Window lost focus
    const Uint32 WINDOWCLOSE = SDL_EVENT_WINDOW_CLOSE_REQUESTED;              // Window was closed
    const Uint32 WINDOWHITTEST = SDL_EVENT_WINDOW_HIT_TEST;         // Window has a special hit test (SDL backend >= 2.0.5)
    const Uint32 WINDOWLAST = SDL_EVENT_WINDOW_LAST;
    ...
    const Uint32 K_DOT = SDL_SCANCODE_KP_PERIOD;
